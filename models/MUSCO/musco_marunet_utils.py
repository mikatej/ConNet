...
def make_layers(cfg, in_channels=3, batch_norm=False, dilation=False, NL='relu', se=False, groups=1, padding=1):
    if dilation:
        d_rate = 2
    else:
        d_rate = 1
        ...
        if v == 'M':
            ...
            if type(v) == list:
                conv2d = nn.Sequential(OrderedDict([
                    ...
                    (str(i) + '-1', nn.Conv2d(v[1][0], v[1][1], kernel_size=(3,3), stride=(1,1), padding=(padding, padding), bias=False)),
